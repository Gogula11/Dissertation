...
**Instance generation.** Synthetic instance generation is necessary because there is no standard benchmark set for PMSP-SDSC with asymmetric costs. The instance generator must produce reproducible instances with controlled properties: processing times derived from weekly capacity, due dates calibrated by a tightness parameter, and asymmetric setup costs structured around colour classes. Processing times are computed as (m × 168) / n hours, where 168 represents weekly operating hours (24/7 continuous operation), and setup times average approximately 1/8 of processing time. This setup-time-to-processing-time ratio is motivated by the real-world observation that vat cleaning takes 1-2 hours while dye cycles take 8-16 hours in textile manufacturing.
...
**Why PPO?** Proximal Policy Optimisation was chosen over alternative DRL algorithms for three reasons. First, its clipped surrogate objective provides stable training without the need for extensive hyperparameter tuning, which is important given the computational cost of training. Second, PPO has been extensively validated on a wide range of continuous-state environments, including those with structured observations similar to our 8-dimensional state space. Third, SB3's PPO implementation is production-quality, well-documented, and includes utilities for TensorBoard logging and model checkpointing.

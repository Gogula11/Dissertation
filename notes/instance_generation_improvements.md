...
| `n_colours`          | 3, 5, 7, 10, 12                      | 7                 | Number of colour families (3 = light/medium/dark; 12 = full)                                                |
...
- `colour_family` (categorical: one of 12)
...
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `evaluator.py`     | `estimate_scales()` — fix normalisation (f1∼f2 scale mismatch). Add `due_date_type="window"` logic. |
| `heuristics.py`    | None (interface unchanged).                                                                          |
| `ga.py`            | None (instance consumed as-is; fitness unchanged).                                                   |
| `ga_env.py`        | Verify obs space sufficient. Likely none — obs is GA-state-based, not instance-state-based.          |
| `drl_agent.py`     | May need more timesteps for richer instances. Requires retraining on new instance profiles.          |
| `experiments/*.py` | Add `--profile` flag. 12 configs × 3 profiles × 30 seeds × 4 methods = 4,320 runs.                  |
...
| 2 | Phase 5 (customer segments)              | Small  | High   | Customer priority realism with minimal code          |
| 3 | Phase 1 (continuous colour + 12 families)| Medium | High   | Core colour model upgrade                            |
...
| 6 | Fix normalisation (estimate_scales)      | Small  | High   | Fixes f1∼f2 scale mismatch, enables meaningful alpha |
| 7 | Phase 8+9 (profiles + validation)        | Medium | High   | Wraps everything, enables clean sensitivity analysis |
| 8 | Phase 4 (decouple time/cost)             | Medium | Medium | Nuance, but lower priority                           |

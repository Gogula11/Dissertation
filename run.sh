...
if [ -z "${VIRTUAL_ENV:-}" ] && [ -z "${CONDA_DEFAULT_ENV:-}" ]; then
    echo "Activate your environment first (e.g., source venv/bin/activate)"
    exit 1
fi
...
mkdir -p results/raw models logs figures results/archives
...
tar czf "results/archives/results_${TIMESTAMP}.tar.gz" results/raw/ models/ logs/ figures/

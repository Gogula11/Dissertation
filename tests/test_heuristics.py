...
def test_spt_returns_all_jobs():
    inst = generate_instance(jobs_per_machine=10, m=1, seed=0)
    sigma = spt(inst)
...
def test_nn_greedy_returns_all_jobs():
    inst = generate_instance(jobs_per_machine=15, m=1, seed=0)
...
def test_nn_greedy_no_empty_machines():
    inst = generate_instance(jobs_per_machine=5, m=3, seed=0)
    sigma = nearest_neighbour_greedy(inst)
    assert all(len(seq) > 0 for seq in sigma)

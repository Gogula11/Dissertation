...
Observation  = 9D vector summarising GA population state and instance context.
...
from src.instance_generator import WEEKLY_HOURS
...
class GAHyperHeuristicEnv(gym.Env):
    ...
    action_space:      Discrete(3)
      ...
      2 = insertion mutation (remove + reinsert)
    """
    ...
    def _obs(self) -> np.ndarray:
        ...
        proc_time = float(inst["proc_times"].mean())
        mean_setup = float(inst["setup_time"][~diag_mask].mean())
        deadline = WEEKLY_HOURS
        util_norm = np.clip((n_jobs * (proc_time + mean_setup)) / (max(self._m_norm * 10.0, 1) * deadline) / 1.5, 0.0, 1.0)
    ...
    def reset(self, seed=None, options=None):
        ...
        self._warmup_remaining = self._warmup_steps
        if seed is not None:
            random.seed(seed)
            np.random.seed(seed)
        ...
        self.instance = self.instance_pool[
            random.randint(0, len(self.instance_pool) - 1)
        ]
    ...
    def step(self, action: int):
        ...
        # Elitism: ensure hof best survives into population for next step
        if self.hof[0] not in self.pop:
            self.pop[-1] = self.toolbox.clone(self.hof[0])
